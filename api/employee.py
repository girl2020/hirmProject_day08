...
class EmployeeApi:
    ...
    # 定义查询员工的方法
    def select_emp(self, emp_id, headers):
        # 拼接查询员工的url
        select_url = self.emp_url + "/" + emp_id
        # 发送查询员工的接口请求,并且返回
        return requests.get(url=select_url, headers=headers)

    # 定义修改员工的方法
    def modify_emp(self, json, emp_id, headers):
        # 拼接修改员工的url
    ...
    def delete_emp(self, emp_id, headers):
        ...
        # 发送修改员工的接口请求,并且返回
        return requests.put(url=delete_url, headers=headers)
